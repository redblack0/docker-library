...
namespace Docker {
    inline void test_for_error(const std::string& err) { if (err != "") throw Docker::ErrorResponseFromDaemon(err); }

    namespace CliCalls {
        // container specific docker requests
        std::string container_create(const std::string& image,const  std::string& network_name,const  std::string& container_ip);
        std::string container_execute(const std::string& name, const std::string& command);
